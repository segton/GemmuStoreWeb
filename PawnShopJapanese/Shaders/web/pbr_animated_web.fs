...
float CalculateMainShadow(vec3 N, vec3 L)
{
    ...
    float darkestShadow = 0.28;

float webShadowStrength = strength * 0.70;

float shadow = mix(darkestShadow, 1.0, visibility);
shadow = mix(1.0, shadow, webShadowStrength);
...
// ---------------------------------------------------------
    ...
    vec3 fakeReflection = vec3(0.04, 0.05, 0.06) * reflectionAmount;

    return ambientFinal + lightAccum * ao + emissive + fakeReflection;
}
